...
- **Klassen verwalten**: Klassen anlegen, umbenennen, löschen; Schüler einzeln oder per Mehrfach-Import (Text) hinzufügen und jederzeit wieder entfernen. Pro Klasse kann ein **Wochenplan** hinterlegt werden (ein oder mehrere Wochentag/Uhrzeit-Termine). Pro Schüler(in) kann markiert werden, ob die Person **sportlich** ist (Läufer-Symbol in der Schülerliste zum Umschalten), und es lassen sich **Hobbys/Sportarten** erfassen (beim Hinzufügen oder über das Ball-Symbol, z.B. "Fussball, Tennis") — diese fliessen in die Team-Generierung ein. Doppelte Vornamen (z.B. zwei "Lena") sind nach Rückfrage möglich — am besten mit Initial unterscheiden ("Lena M").
...
- **Import/Export**: Klassen (inkl. Wochenplan, Sportlich-Markierung und Hobbys) als JSON exportieren und wieder importieren (auch CSV/TXT im Format `Klasse;Name;Geschlecht;Sportlich;Kürzel;Hobbys` — Sportlich optional: `s`/`ja`/`x`/`1`; Hobbys mit `/` getrennt; eine Kopfzeile wird automatisch erkannt und übersprungen).
...
- **Anwesenheitscheck**: Pro Klasse und Datum wird eine eigene Anwesenheits-Sitzung erfasst (mehrere Termine pro Klasse möglich, z.B. jede Woche). Klicken wechselt den Status: **Anwesend (grün) → Abwesend (rot) → Verletzt (orange)** — Verletzt heisst: anwesend, kann aber nicht aktiv mitmachen (z.B. verletzt); diese Schüler(innen) zählen als anwesend, werden aber nicht in die Team-Generierung übernommen. Bei Abwesenheit kann ein Grund angegeben werden (Kategorie: Krank / Verletzt / Entschuldigt / Schnuppern / Unentschuldigt / Sonstiges, plus optionale Notiz). Passt das gewählte Datum nicht zum hinterlegten Wochenplan, wird ein Hinweis angezeigt. Erfasste Termine lassen sich in der Terminliste erneut öffnen, bearbeiten oder löschen (mit Rückgängig-Option). **Ungespeicherte Änderungen** werden angezeigt und vor Verlust (Datums-/Klassenwechsel, Schliessen der Seite) gewarnt.
- **Auswertung**: Pro Klasse eine Übersicht mit Anwesenheits-/Abwesenheitszahlen, Anwesenheitsquote, Anzahl erfasster Termine pro Schüler(in) und Aufschlüsselung der Abwesenheitsgründe (verletzt Anwesende zählen zur Quote, werden aber separat ausgewiesen) – inkl. Durchschnittszeile, sortierbaren Spalten und CSV-Export. Lässt sich nach **Semester 1 (Aug–Jan) oder Semester 2 (Feb–Jul)** und — wenn die Gruppe Klassenkürzel nutzt — nach **Klassenkürzel** filtern. Aus der Klasse entfernte Schüler(innen) mit erfasster Historie bleiben als **"ehemalig"** in der Auswertung sichtbar.
...
- **Punkte-Tracker (Bonusstunden)**: Pro Klasse aktivierbar (beim Anlegen oder in der Detailansicht). Pro Lektion werden erfüllte Ziele abgehakt (z.B. "Alle starten sofort", "Fairplay") — jedes Ziel gibt wahlweise **1 oder 2 Punkte**, die Ziele sind frei anpassbar. Der Punktestand summiert sich über die Lektionen und wird als **Verlaufs-Graph** angezeigt; über "Punkte einlösen" wird er (z.B. für eine Bonusstunde) auf 0 zurückgesetzt, der Verlauf bleibt nachvollziehbar. Über **"Punkte anpassen"** lassen sich Punkte pauschal hinzufügen oder abziehen (z.B. Startpunktestand beim Umstieg vom alten Tracker; der Stand fällt nie unter 0). Lektionen können Bemerkungen tragen und nachträglich gelöscht werden.
- **Teams generieren**: Zufällige Teams aus den anwesenden bzw. manuell hinzugefügten Personen erstellen — wahlweise nach **Anzahl Teams oder Personen pro Team** — kopieren, **drucken** und neu mischen. Über die Auswahl **"Wofür sind die Gruppen?"** wird die Verteilung gesteuert: *Kein Sport* (z.B. Klassenzimmer — keine Sport-Balance), *Sport — allgemeines Spiel* (Sportlich-Markierung fair verteilen) oder eine **konkrete Sportart** (erscheint automatisch, sobald Hobbys erfasst sind — dann werden zusätzlich die Schüler(innen) mit diesem Hobby fair verteilt). Optional werden auch Geschlechter fair verteilt. Sportlich-Markierung und Hobbys sind dabei bewusst **nirgends im Teams-Tab sichtbar**, damit sie beim Projizieren vor der Klasse nicht erkennbar sind. Über **"Nicht zusammen"-Regeln** lassen sich Paare definieren, die nicht im gleichen Team landen sollen.
...
[
  {
    ...
    "students": [
      { "name": "Anna", "gender": "female", "sporty": true, "hobbies": ["Fussball", "Tennis"] },
